...
const FlaggedUsers = () => {
  const { data, isLoading, isSuccess, refetch } = useGetFlaggedQuery();
      ...
      },
      {
        ...
        Cell: (row) => (
          <Menu placement="bottom-start">
            ...
            <MenuList>
              {checkArray(
                row?.row.original?.flagged,
                "fraud"
              ) && (
                <MenuItem
                  onClick={() => unFlagUser("fraud", row.value)}
                >
                  Unflag Fraud
                </MenuItem>
              )}
              {checkArray(
                ...
                <MenuItem onClick={() => unFlagUser("documentation", row.value)}>
